...
################### BUILD FOR SIM
ICU_IOS_SIM_NATIVE_BUILD_FOLDER=$ICU_VER_NAME-ios.sim-native-build
if [ ! -f $ICU_IOS_SIM_NATIVE_BUILD_FOLDER.success ]; then
echo preparing build folder $ICU_IOS_SIM_NATIVE_BUILD_FOLDER ...
if [ -d $ICU_IOS_SIM_NATIVE_BUILD_FOLDER ]; then
    rm -rf $ICU_IOS_SIM_NATIVE_BUILD_FOLDER
fi
cp -r $ICU4C_FOLDER $ICU_IOS_SIM_NATIVE_BUILD_FOLDER
echo "building icu (iOS: iPhoneSimulator)..."
pushd $ICU_IOS_SIM_NATIVE_BUILD_FOLDER/source
...
make -j$THREAD_COUNT
popd
touch $ICU_IOS_SIM_NATIVE_BUILD_FOLDER.success 
fi
